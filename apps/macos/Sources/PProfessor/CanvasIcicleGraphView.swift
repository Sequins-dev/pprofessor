...
/// Canvas-based icicle graph visualization.
struct CanvasIcicleGraphView: View {
    let nodes: [FlamegraphNode]
    let nodeIndex: [String: Int]
    ...
    @State private var pinchStartContentX: CGFloat = 0
    @State private var mouseContentX: CGFloat = 0

    var displayRootId: String { rootNodeId ?? "root" }
    ...
    var body: some View {
        ...
        ScrollView([.horizontal, .vertical], showsIndicators: true) {
            ...
            .frame(width: contentWidth, height: max(contentHeight, availableHeight))
    ...
    private func getNode(_ id: String) -> FlamegraphNode? {
        guard let idx = nodeIndex[id], idx < nodes.count else { return nil }
        return nodes[idx]
    }
}

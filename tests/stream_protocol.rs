...
use pprofessor::{RawProfile, RawProfileCursor};
...
fn raw_profile_cursor_returns_only_unpublished_counts() {
    ...
    let mut profile = RawProfile {
        stacks: HashMap::from([((1, vec![0x10, 0x20]), 3)]),
    ...
    assert_eq!(first.stacks[&(1, vec![0x10, 0x20])], 3);

    profile.stacks.insert((1, vec![0x10, 0x20]), 5);
    let second = cursor.delta(&profile).unwrap();
    assert_eq!(second.stacks[&(1, vec![0x10, 0x20])], 2);
